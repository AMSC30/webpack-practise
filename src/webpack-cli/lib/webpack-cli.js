...
class WebpackCLI {
    ...
    async doInstall(packageName, options = {}) {
        ...
        try {
            needInstall = await prompt({
                message: `[webpack-cli] Would you like to install '${this.colors.green(packageName)}' package? (That will run '${this.colors.green(
                    commandToBeRun
    ...
    async tryRequireThenImport(module, handleError = true) {
        ...
        try {
            ...
            if ((error.code === 'ERR_REQUIRE_ESM' || process.env.WEBPACK_CLI_FORCE_LOAD_ESM_CONFIG) && pathToFileURL && dynamicImportLoader) {
        ...
        if (commandOptions.dependencies && commandOptions.dependencies.length > 0) {
            for (const dependency of commandOptions.dependencies) {
                ...
                await this.doInstall(dependency, {
                    preMessage: () => {
                        this.logger.error(
                            `For using '${this.colors.green(commandOptions.name.split(' ')[0])}' command you need to install: '${this.colors.green(dependency)}' package.`
                        )
                    }
                })
            }
    ...
    async run(args, parseOptions) {
        ...
        const knownCommands = [buildCommandOptions, watchCommandOptions, versionCommandOptions, helpCommandOptions, ...externalBuiltInCommandsInfo]
        const getCommandName = name => name.split(' ')[0]
        const isKnownCommand = name =>
            knownCommands.find(command => getCommandName(command.name) === name || (Array.isArray(command.alias) ? command.alias.includes(name) : command.alias === name))
        ...
        const isGlobalOption = value => value === '--color' || value === '--no-color' || value === '-v' || value === '--version' || value === '-h' || value === '--help'
            ...
            if (isBuildCommandUsed || isWatchCommandUsed) {
                await this.makeCommand(
                    ...
                    async () => {
                        ...
                        return isWatchCommandUsed ? this.getBuiltInOptions().filter(option => option.name !== 'watch') : this.getBuiltInOptions()
                    },
        ...
        const outputHelp = async (options, isVerbose, isHelpCommandSyntax, program) => {
            ...
            if (isGlobalHelp || isCommandHelp) {
                ...
                const nameOutput = option.flags.replace(/^.+[[<]/, '').replace(/(\.\.\.)?[\]>].*$/, '') + (option.variadic === true ? '...' : '')
    ...
    async loadConfig(options) {
        ...
        if (options.configName) {
            ...
            if (notFoundConfigNames.length > 0) {
                this.logger.error(notFoundConfigNames.map(configName => `Configuration with the name "${configName}" was not found.`).join(' '))
                process.exit(2)
            }
        }
        if (options.merge) {
    ...
    async buildConfig(config, options) {
        ...
        const internalBuildConfig = item => {
            // Output warnings
            if (item.watch && options.argv && options.argv.env && (options.argv.env['WEBPACK_WATCH'] || options.argv.env['WEBPACK_SERVE'])) {
                this.logger.warn(
                    `No need to use the '${options.argv.env['WEBPACK_WATCH'] ? 'watch' : 'serve'}' command together with '{ watch: true }' configuration, it does not make sense.`
                )
                ...
                if (problems) {
                    ...
                    for (const path in problemsByPath) {
                        ...
                        problems.forEach(problem => {
                            this.logger.error(
                                `${this.capitalizeFirstLetter(problem.type.replace(/-/g, ' '))}${problem.value ? ` '${problem.value}'` : ''} for the '--${
                                    problem.argument
    ...
    async runWebpack(options, isWatchCommand) {
        ...
        const env =
            isWatchCommand || options.watch ? Object.assign({ WEBPACK_WATCH: true }, options.env) : Object.assign({ WEBPACK_BUNDLE: true, WEBPACK_BUILD: true }, options.env)
        ...
        const isWatch = compiler => Boolean(this.isMultipleCompiler(compiler) ? compiler.compilers.some(compiler => compiler.options.watch) : compiler.options.watch)
